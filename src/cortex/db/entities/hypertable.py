...
class Estop(Base):
    ...
    state_string = Column(String, nullable=True, )

    def __repr__(self):
        return f"""<Estop(id='{self.id}', time='{self.time}', msg_time='{self.msg_time}', robot='{self.robot}', name='{self.name}', type='{self.type}', state='{self.state}', state_string='{self.state_string}')>"""


class SignalNoiseRatio(Base):
    __tablename__ = 'ts_signal_noise_ratio'
    __table_args__ = ({'timescaledb_hypertable': {'time_column_name': 'time', 'chunk_time_interval': '1 day'}})
    id = Column(Integer, primary_key=True, autoincrement=True)
    time = Column(DateTime(timezone=True), primary_key=True, nullable=False)
    msg_time = Column(DateTime(timezone=True), nullable=True)
    ...
    snr = Column(DOUBLE_PRECISION, nullable=False)

    def __repr__(self):
        return f"""<SignalNoiseRatio(id='{self.id}', time='{self.time}', msg_time='{self.msg_time}', robot='{self.robot}', iface='{self.iface}', snr='{self.snr}')>"""
