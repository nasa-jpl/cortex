...
class TestBasicCRTXWorker(unittest.TestCase):
    ...
    def tearDown(self):
        with self.db.get_session() as session:
            ...
            session.query(JointStatesActual).where(
                JointStatesActual.robot == "Test Robot"
            ).delete()
            session.commit()
    ...
    def test_basic_config(self):
        ...
        for i in range(3):
            entity = entities[i]
            self.assertEqual(
                entity.robot, "Test Robot", "Robot name should be Test Robot"
            )
            self.assertEqual(
                entity.time, self.right_now, f"time should be {self.right_now}"
            )
            self.assertEqual(
                entity.msg_time, self.right_now, f"msg_time should be {self.right_now}"
            )
            self.assertEqual(
                entity.actuator,
                self.data.get("name")[i],
                f"actuator should be {self.data.get('name')[i]}",
            )
            self.assertEqual(
                entity.velocity,
                self.data.get("velocity")[i],
                f"velocity should be {self.data.get('velocity')[i]}",
            )
            self.assertEqual(
                entity.position,
                self.data.get("position")[i],
                f"position should be {self.data.get('position')[i]}",
            )
            self.assertEqual(
                entity.effort,
                self.data.get("effort")[i],
                f"effort should be {self.data.get('effort')[i]}",
            )
